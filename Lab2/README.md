...
- **parser**
  ...
  - parser_Embedding_based.py
    ...
    - --kg_l2loss_lambda：计算loss时的参数，默认1e-4，因为他的loss = dis + lambda * len，加了个正则项
    - --lr：学习率，默认1e-3
    - --n_epoch：训练轮数，默认1000
...
- 首先我们现在拥有的是三元的数据集（HEAD，RELAT， TAIL），比如（1， 2， 3）表示 1 通过关系 2 得到结果 3
- 我们希望做的是：找到三个n维的向量，来分别代表这三个量，并且让他们尽可能满足 v_1 + v_2 = v_3
- 所以我们把HEAD + TAIL与RELAT中的每个元素定义一个k维向量，并且把他们拼成一个 （h + t） * k 跟一个 r * k 的大矩阵作为我们网络优化的目标
- 然后就是正常的利用这两个参数矩阵计算一个loss，反向传播回来优化对应的参数，最后得到的就是两个优化好的参数矩阵

**TransR看起来是给RELAT换了个维度，然后对应的进行一些操作巴拉巴拉的......**
