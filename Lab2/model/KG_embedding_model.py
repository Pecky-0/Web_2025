...
class Embedding_based(nn.Module):
    ...
    def calc_kg_loss_TransR(self, h, r, pos_t, neg_t):
        ...
        r_mul_h = F.normalize(h_embed, p=2, dim=1)
        r_mul_pos_t = F.normalize(pos_t_embed, p=2, dim=1)
        r_mul_neg_t = F.normalize(neg_t_embed, p=2, dim=1)
        ...
        kg_loss = -F.logsigmoid(neg_score - pos_score)                        # 基于pos_score，neg_score，补全BPR loss
    ...
    def calc_kg_loss_TransE(self, h, r, pos_t, neg_t):
        ...
        kg_loss = -F.logsigmoid(neg_score - pos_score)
        kg_loss = torch.mean(kg_loss)
